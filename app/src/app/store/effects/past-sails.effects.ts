...
export class PastSailsEffects {
  ...
  fetchPastSailsForUser$ = createEffect(
    () => this.actions$.pipe(
      ...
      tap(() => this.store.dispatch(startLoading())),
      mergeMap(
        action => this.sailService.fetchPastSailsForUser(action.id, action.query)
          .pipe(
            mergeMap((sails) => {
              if (action.notify) {
                return of(
                  putPastSailsForUser({ sails, id: action.id }),
                  putSnack({ snack: { type: SnackType.INFO, message: 'Refreshed' } }),
                );
              }
              return of(putPastSailsForUser({ sails, id: action.id }));
            }),
