  ...
  cancelSailRequest$ = createEffect(
    () => this.actions$
      .pipe(
        ...
        tap(() => this.store.dispatch(startLoading())),
        mergeMap(
          action => this.sailRequestService.update(action.id, { status: SAIL_REQUEST_STATUS.CANCELLED })
            .pipe(
              mergeMap(sailRequest => of(
                putSailRequest({ sailRequest, id: sailRequest.id }),
  ...
  expireSailRequest$ = createEffect(
    () => this.actions$
      .pipe(
        ...
        tap(() => this.store.dispatch(startLoading())),
        mergeMap(
          action => this.sailRequestService.update(action.id, { status: SAIL_REQUEST_STATUS.EXPIRED })
            .pipe(
              mergeMap(sailRequest => of(
                putSailRequest({ sailRequest, id: sailRequest.id }),
  ...
  scheduleSailRequest$ = createEffect(
    () => this.actions$
      .pipe(
        ...
        tap(() => this.store.dispatch(startLoading())),
        mergeMap(
          action => this.sailRequestService.update(action.id, { status: SAIL_REQUEST_STATUS.SCHEDULED })
            .pipe(
              mergeMap(sailRequest => of(
                putSailRequest({ sailRequest, id: sailRequest.id }),
