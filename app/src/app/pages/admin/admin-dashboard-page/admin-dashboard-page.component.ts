...
export class AdminDashboardPageComponent extends BasePageComponent implements OnInit, AfterViewInit {
  ...
  public listLogsRoute = listLogsRoute;
  ...
  public get pendingApproval(): IProfile[] {
    const profiles = this.profiles || {} as IProfileMap;
    const profileIds = Object.keys(profiles);
    const pendingProfiles = profileIds
      .filter(id => !!profiles[id])
      .filter(id => profiles[id].status === PROFILE_STATUS.PENDING_APPROVAL)
      .map(id => profiles[id]);
  ...
  public fetchPendingProfiles(notify?: boolean): void {
    this.dispatchAction(fetchProfiles({ notify, query: `status=${PROFILE_STATUS.PENDING_APPROVAL}` }));
  }

  public editProfilePrivileges(profile: IProfile): void {
